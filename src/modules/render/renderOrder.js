...
const showOrderInfo = (data) => {
  ...
  modalBody.insertAdjacentHTML('beforeend', `
       <h2 class="modal__title">№${data.id}</h2>
    ...
    <ul class="modal__customer-data customer">
      ...
      ${data.address && ` <li class="customer__item">
      ...
      ${data.email && ` <li class="customer__item">
  ...
  const goodsList = createElement('ul', {
    className: 'modal-goods goods-list',
  }, {parent: modalBody, appends: [...data.order.map(item => {
  ...
  createElement('p', {
    ...
    textContent: 'ten '
  }, {parent: cartTotal, append: createElement('span', {}, {cb(elem) {
...
export const renderOrder = ({ render }) => {
  ...
  orderForm.insertAdjacentHTML('beforeend',
    ...
    <label class="order__label">
      <input class="order__input" type="text" placeholder="Адресс доставки" name="address">
    </label>
    <label class="order__label">
